...
//       <div class="report-hero">
//         <div class="container">
//           <!-- ✅ Uses navigateTo directly via onclick — avoids data-nav delegation timing issues -->
//           <a class="report-back-btn" href="#" id="report-back-dashboard">← Back to Dashboard</a>
//           <div class="report-hero-top">
//             <div>
//               <div class="text-label text-muted mb-2">
//                 ${personality ? `Reviewed by ${personality.name}` : 'AI Analysis'} · ${formData.roastLevel?.toUpperCase()} mode
...
//     // ✅ Wire back button directly after render — no dependency on global data-nav delegation
//     container.querySelector('#report-back-dashboard')?.addEventListener('click', (e) => {
//       e.preventDefault();
//       window.navigateTo?.('dashboard');
//     });
...
window.ReportManager = (function () {
  ...
  function render(formData, report) {
    ...
    // ✅ Save to Dashboard — require login first
    container.querySelector('#report-save-btn')?.addEventListener('click', () => {
      if (!window.AuthManager?.isLoggedIn()) {
        // Store a pending save flag so we can complete it after login
        window._pendingSave = { formData, report };
        window.showToast?.('Sign in to save your report to the dashboard', 'info');
